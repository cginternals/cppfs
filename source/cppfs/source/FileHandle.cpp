...
FileHandle::FileHandle() : m_backend(nullptr) {}
...
FileHandle::FileHandle(const FileHandle &fileHandle)
    : m_backend(fileHandle.m_backend ? fileHandle.m_backend->clone()
                                     : nullptr) {}
...
std::unique_ptr<Tree> FileHandle::readTree(const std::string &path,
                                           bool includeHash) const {
  // Check if file or directory exists
  if (!exists()) {
    return nullptr;
  }
...
FileIterator FileHandle::end() const { return FileIterator(); }
...
FileHandle FileHandle::parentDirectory() const {
  return m_backend ? m_backend->fs()->open(
                         FilePath(m_backend->path()).resolve("..").resolved())
                   : FileHandle();
}

FileHandle FileHandle::open(const std::string &path) const {
  return m_backend ? m_backend->fs()->open(
                         FilePath(m_backend->path()).resolve(path).fullPath())
                   : FileHandle();
}
...
FileHandle::createInputStream(std::ios_base::openmode mode) const {
  // Check backend
  if (!m_backend) {
    return nullptr;
  }
...
FileHandle::createOutputStream(std::ios_base::openmode mode) {
  // Check backend
  if (!m_backend) {
    return nullptr;
  }
